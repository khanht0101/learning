...
class ContentColumnItem extends React.Component {
  ...
  renderFourColumns(item) {
    ...
    if (item.imageUrls.length > 1) {
      images = <img className="product-block__image" src={item.imageUrls[0]} alt={item.imageAlt} onMouseOver={e => (e.currentTarget.src = item.imageUrls[1])} onMouseOut={e => (e.currentTarget.src = item.imageUrls[0])} />
    } else {
      images = <img className="product-block__image" src={item.imageUrl} alt={item.imageAlt} />
    }
      ...
      <div className="col-xs-6 col-sm-6 col-md-3 col-lg-3">
            <div className="product-block text-center">
                <a href={item.url} className="product-block__nav">
                    {images}       
                    <span className="product-block__name responsive-title product-title">{item.title}</span>
                    {item.secondaryTitle && <span className="product-block__name responsive-title product-title">{item.secondaryTitle}</span>}
                    {item.thirdTitle && <span className="product-block__name responsive-title product-title">{item.thirdTitle}</span>}
  ...
  renderFourColumnsWithoutDescription(item) {
    let images = '';
    if (item.imageUrls.length > 1) {
      images = <img className="product-block__image" src={item.imageUrls[0]} alt={item.imageAlt} onMouseOver={e => (e.currentTarget.src = item.imageUrls[1])} onMouseOut={e => (e.currentTarget.src = item.imageUrls[0])} width="242" height="242" />
    } else {
      images = <img className="product-block__image" src={item.imageUrl} alt={item.imageAlt} width="242" height="242" />
    }
  ...
  render() {
    ...
    switch (this.props.itemView) {
      ...
      case CONTENT_LISTING_LAYOUTS.fourColumns:
        ...
        case CONTENT_LISTING_LAYOUTS.fourColumnsWithoutDescription:
          return this.renderFourColumnsWithoutDescription(item);
      default:
        return this.renderTwoColumns(item);
    }
  }
